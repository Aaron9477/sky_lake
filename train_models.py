...
def m():
    ...
    for mall_id in malls:
        ...
        cur.execute(sql)
            ...
            # conn.commit()
            # cur.execute(sql)
        n_est = int(cur.fetchone()[0])
        print(get_time(), ' n_est : ', n_est )
        ...
        data, tar = u.get_data_loc(mall_id)
        clf.fit(data,tar)
        save_dir = root_path + "model/" + clf_name + "_" + mall_id + "_model.m"  # 存储模型位置
        joblib.dump(clf, save_dir)
        print(get_time(), ' saved a model for ', mall_id)
        ...
        # clf_name = "xgboost"
        # save_dir = root_path + "model/" + clf_name + "_" + mall_id + "_model.m"  # 存储模型位置
        # sql = "SELECT param from `score_xgb_17.11.4` WHERE mall_id='{m}'".format(m=mall_id)
        # cur.execute(sql)
        # conn.commit()
        # n_est = int(cur.fetchall()[0][0])  #迭代次数设置
        # if n_est >= 120:
        #     n_est = 120
        #
        # print(get_time(), 'iterate times : ', n_est)
        ...
        # cur.execute(sql)
        # conn.commit()
        # print('mall : {m:6} | score : {s:10} | iterate times : {t} '.format(m=mall_id, s=score, t=n_est))
        # joblib.dump(clf, save_dir)
    ...
    # algs = ['knn_5','DT','SGD','MNB','GNB','BNB','GBM']
    # for a in algs:
    #     examinate(a)
    #     print (a, ' training starts')
    # 随机森林
    # print('RF_1000 training starts')
    # examinate('RF_1000')
